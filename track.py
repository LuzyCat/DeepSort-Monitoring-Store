...
def detect(opt):
    ...
    for frame_idx, (path, im, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            img_path = str(Path(save_path).with_suffix('.png'))
                ...
                if len(outputs[i]) > 0:
                    ...
                    for j, (output, conf) in enumerate(zip(outputs[i], confs)):
                        ...
                        if save_vid or save_crop or show_vid:  # Add bbox to image
                            ...
                            px, py = getCentralPoint(bboxes, (0, 0), opt.foot)
            ...
            if show_vid:
                if image_list[i].IsSelected:
                    ...
                    image_list[i].draw_ROI(im0, opt.capacity)
                    image_list[i].draw_line(im0)
                if opt.trajectory:
                    im0 = cv2.addWeighted(im0, 1.0, image_list[i].mask_traj, 0.5, 0)
                # if opt.heatmap:
                ...
                if key == ord('q'):  # q to quit
                    ...
                    opt.trajectory = not opt.trajectory
                elif key == ord('c'):
                    cv2.imwrite(img_path, imo)
                elif key == ord('h'):
                    if Path(img_path).is_file() and Path(txt_path + '.txt').is_file():
                        LOGGER.info(f"Generate Heatmap...")
                        heatmapThread = threading.Thread(target=makeHeatmap, args=(img_path, txt_path + '.txt'))
                        heatmapThread.start()
                        # makeHeatmap(img_path, txt_path + '.txt')
